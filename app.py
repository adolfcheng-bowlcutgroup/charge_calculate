...
with st.sidebar:
    ...
    st.header("3. 軌道一：場地租金回饋設定")
    st.info("設定各等級下，撥出多少 **票券營收** 給 A 公司")
    
    # 5 個等級的滑桿 (維持不變)
    p1 = st.slider("Lv1. 租金減免 0~20% (回饋%)", 0.0, 5.0, 1.0, 0.1) / 100
    p2 = st.slider("Lv2. 租金減免 21~40% (回饋%)", 0.0, 8.0, 2.5, 0.1) / 100
    p3 = st.slider("Lv3. 租金減免 41~60% (回饋%)", 0.0, 10.0, 4.0, 0.1) / 100
    p4 = st.slider("Lv4. 租金減免 61~80% (回饋%)", 0.0, 12.0, 6.0, 0.1) / 100
    p5 = st.slider("Lv5. 租金減免 81~100% (回饋%)", 0.0, 15.0, 8.0, 0.1) / 100

    st.divider()
    
    # --- 修改部分開始：軌道二變為變數 ---
    st.header("4. 軌道二：商品抽成設定")
    st.markdown("設定 A 公司談到的 **抽成減免幅度**：")
...
# --- Chart: 區間四象限圖 ---
st.subheader(f"🎯 雙軌情境分析 (當商品減免 {merch_reduction_pct}% 時)")
