...
with st.sidebar:
    ...
    # --- 修改部分：11 個等級的滑桿 ---
    # 為了版面整潔，預設值做了平滑處理
    p0  = st.slider("Lv0. 租金減免 0% (無折扣)", 0.0, 5.0, 0.0, 0.1) / 100
    p1  = st.slider("Lv1. 租金減免 1~10%", 0.0, 5.0, 0.5, 0.1) / 100
    p2  = st.slider("Lv2. 租金減免 11~20%", 0.0, 6.0, 1.0, 0.1) / 100
    p3  = st.slider("Lv3. 租金減免 21~30%", 0.0, 7.0, 1.5, 0.1) / 100
    p4  = st.slider("Lv4. 租金減免 31~40%", 0.0, 8.0, 2.5, 0.1) / 100
    p5  = st.slider("Lv5. 租金減免 41~50%", 0.0, 9.0, 3.5, 0.1) / 100
    p6  = st.slider("Lv6. 租金減免 51~60%", 0.0, 10.0, 4.5, 0.1) / 100
    p7  = st.slider("Lv7. 租金減免 61~70%", 0.0, 11.0, 5.5, 0.1) / 100
    p8  = st.slider("Lv8. 租金減免 71~80%", 0.0, 12.0, 6.5, 0.1) / 100
    p9  = st.slider("Lv9. 租金減免 81~90%", 0.0, 13.0, 7.5, 0.1) / 100
    p10 = st.slider("Lv10. 租金減免 91~100%", 0.0, 15.0, 8.5, 0.1) / 100
    # --- 修改部分結束 ---
